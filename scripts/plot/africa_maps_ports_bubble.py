...
def main(config):
    ...
    IWW_nodes = IWW_nodes[~IWW_nodes["infra"].isin(["IWW route"])]
    ...
    maritime_nodes = maritime_nodes.sort_values(
        by="vessel_count_total", ascending=False
    )
    ...
    ins.text(
        xt, yt, "Port annual vessel count", weight="bold", va="center", fontsize=12
    )
    for k in range(Nk):
    ...
    for infra_type in IWW_nodes["infra"].unique():
        ...
        IWW_nodes[IWW_nodes["infra"] == infra_type].plot(
            ax=ax, zorder=4, color=color, markersize=10, label=infra_type
        )

    maritime_edges.plot(
        ax=ax, zorder=3, color="#3690c0", linewidth=1, label="Maritime route"
    )

    maritime_nodes.plot(
        ax=ax, zorder=4, color="#3690c0", markersize=25, label="Maritime port"
    )
