...
def main():
    ...
    for jdx, (sc_n, sc_t, gdf, rowpos, colpos, rowspan, colspan) in enumerate(sc_dfs):
        ax = plt.subplot2grid(
            [2, 5], [rowpos, colpos], rowspan=rowspan, colspan=colspan
        )
        ...
        if sc_n == "key":
            ...
            ax.text(
                0.95 * xl[0], 0.90 * yl[1], sc_t, fontsize=18, weight="bold", ha="left"
            )
    ...
    for row in df.itertuples():
        ...
        if row.Total > threshold:
            ax.text(
                row.geometry.x, row.geometry.y, row.ADM0_A3, ha="center", fontsize=5.5
            )
