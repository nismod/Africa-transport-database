...
def main(config):
    ...
    input_descriptions = [
        ...
        },
        {
            ...
            "project_attributes": {
                "country": "Tanzania-Rwanda-Burundi",
                "status": "proposed",
                "mode": "mixed",
                "gauge": 1435,
        ...
        },
        {
            ...
            "project_attributes": {
                "country": "Kenya-Uganda-DRC-Rwanda-Burundi-South Sudan-Ethopia",
                "status": "proposed",
                "mode": "mixed",
                "gauge": 1435,
        ...
        for inputs in input_descriptions:
            ...
            if inputs["points_file"] is not None:
                df_points = gpd.read_file(
                    os.path.join(rail_paths, inputs["points_file"]),
                    layer=inputs["points_layer"],
                )
                df_points.columns = map(str.lower, df_points.columns)
            elif inputs["project_name"] == "Standard Gauge Railway":
                tza_sgr_nodes = gpd.read_file(
                    os.path.join(rail_paths, "tanzania_standard_gauge_railway.gpkg"),
                    layer="nodes",
                )
                isaka_sgr_nodes = gpd.read_file(
                    os.path.join(rail_paths, "isaka_kigali_gitega_railway.gpkg"),
                    layer="nodes",
                )
                df_points = pd.concat(
                    [tza_sgr_nodes, isaka_sgr_nodes], axis=0, ignore_index=True
                )
                df_points.drop(["node_id", "component"], axis=1, inplace=True)
                df_points.columns = map(str.lower, df_points.columns)
                df_points = gpd.GeoDataFrame(
                    df_points, geometry="geometry", crs="EPSG:4326"
                )
            else:
                df_points = inputs["points_file"]

            if inputs["project_name"] == "Standard Gauge Railway":
                tza_sgr_lines = gpd.read_file(
                    os.path.join(rail_paths, "tanzania_standard_gauge_railway.gpkg"),
                    layer="edges",
                )
                isaka_sgr_lines = gpd.read_file(
                    os.path.join(rail_paths, "isaka_kigali_gitega_railway.gpkg"),
                    layer="edges",
                )
                df_lines = pd.concat(
                    [tza_sgr_lines, isaka_sgr_lines], axis=0, ignore_index=True
                )
                df_lines.drop(
                    ["edge_id", "from_node", "to_node", "component"],
                    axis=1,
                    inplace=True,
                )
                df_lines = gpd.GeoDataFrame(
                    df_lines, geometry="geometry", crs="EPSG:4326"
                )
            else:
                df_lines = gpd.read_file(
                    os.path.join(rail_paths, inputs["lines_file"]),
                    layer=inputs["lines_layer"],
                )
            ...
            if inputs["project_name"] == "Conakry-Kankan Railway":
                ...
                df_lines["status"] = df_lines.progress_apply(
                    lambda x: get_line_status(str(x["comments"]).lower()), axis=1
                )
                df_lines["country"] = "Guinea"
                df_lines["mode"] = np.where(
                    df_lines["type2"] == "Freight & Passenger", "mixed", "freight"
                )
                df_lines["gauge"] = 1000
                df_lines = df_lines[
                    ["country", "line", "status", "mode", "gauge", "geometry"]
                ]
            elif inputs["project_name"] in [
            ...
            for c in range(len(all_components) - 1):
                for p in range(c + 1, len(all_components)):
                    c1 = nodes[nodes["component"] == all_components[c]][
                        ["node_id", "geometry"]
                    ]
                    c2 = nodes[nodes["component"] == all_components[p]][
                        ["node_id", "geometry"]
                    ]
                    ...
                    if len(distances.index) > 0:
                        ...
                        distances.rename(
                            columns={"node_id": "to_node", "geometry": "to_geometry"},
                            inplace=True,
                        )
                        distances["geometry"] = distances.progress_apply(
                            lambda x: LineString([x.from_geometry, x.to_geometry]),
                            axis=1,
                        )
                        distances["edge_id"] = list(
                            max_edge_id + 1 + distances.index.values
                        )
                        distances["edge_id"] = distances.progress_apply(
                            lambda x: f"e_{x.edge_id}", axis=1
                        )
                        distances.drop(
                            ["from_geometry", "to_geometry", "dist"],
                            axis=1,
                            inplace=True,
                        )
                        distances = add_attributes(
                            distances, inputs["project_attributes"]
                        )
                        distances = gpd.GeoDataFrame(
                            distances, geometry="geometry", crs=epsg_meters
                        )
            ...
            if len(component_dfs) > 0:
                edges = pd.concat([edges] + component_dfs, axis=0, ignore_index=True)
            ...
            edges, nodes = components(
                edges, nodes, "node_id", "edge_id", "from_node", "to_node"
            )
            output_file_name = inputs["project_name"].lower().replace(" ", "_")
            gpd.GeoDataFrame(nodes, geometry="geometry", crs=df_lines.crs).to_file(
                os.path.join(rail_paths, f"{output_file_name}.gpkg"),
                layer="nodes",
                driver="GPKG",
            )
            gpd.GeoDataFrame(edges, geometry="geometry", crs=df_lines.crs).to_file(
                os.path.join(rail_paths, f"{output_file_name}.gpkg"),
                layer="edges",
                driver="GPKG",
            )
    ...
    # Other databases
    input_descriptions = [
        {"project_name": "Guinea Lines", "find_nearest": False},
        {"project_name": "Simandou railway project", "find_nearest": False},
        {"project_name": "Togo Rail", "find_nearest": False},
        {"project_name": "Ghana Burkina Faso", "find_nearest": False},
        {"project_name": "Standard Gauge Railway", "find_nearest": False},
        {
            "project_name": "East Africa Railway",
            "find_nearest": True,
            "project_attributes": {
    ...
    if processinng_step_two is True:
        ...
        select_ids = list(
            set(
                rail_edges["source"].values.tolist()
                + rail_edges["target"].values.tolist()
            )
        )
        ...
        for inputs in input_descriptions:
            ...
            df_nodes = gpd.read_file(
                os.path.join(rail_paths, f"{output_file_name}.gpkg"), layer="nodes"
            )
            ...
            df_edges = gpd.read_file(
                os.path.join(rail_paths, f"{output_file_name}.gpkg"), layer="edges"
            )
            ...
            if inputs["find_nearest"] is True:
                ...
                if len(same_points.index) > 0:
                    ...
                    df_nodes = df_nodes[
                        ~df_nodes.oid.isin(same_points.oid.values.tolist())
                    ]
                distances = dist[
                    (dist["dist"] > 1.0e-2) & (dist["dist"] <= distance_threshold)
                ]
                if len(distances.index) > 0:
                    distances.columns = ["source", "from_geometry", "oid", "dist"]
                    distances = pd.merge(distances, c2, how="left", on=["oid"])
                    distances.rename(
                        columns={"oid": "target", "geometry": "to_geometry"},
                        inplace=True,
                    )
                    distances["geometry"] = distances.progress_apply(
                    ...
                    distances.drop(
                        ["from_geometry", "to_geometry", "dist"], axis=1, inplace=True
                    )
        ...
        edges, nodes = components(
            all_edges, all_nodes, "oid", "oid", "source", "target"
        )
