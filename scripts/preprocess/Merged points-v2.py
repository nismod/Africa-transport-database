...
def main(config):
    ...
    merged_three = gpd.sjoin(
        merged_shp_gpkg, merged_shp_corridor, how="inner", predicate="intersects"
    )
    # 5. Delete the duplicates
    duplicates_gpkg = merged_shp_gpkg[
        merged_shp_gpkg.duplicated(subset="geometry", keep=False)
    ]
    print("Duplicated number of merged_shp_gpkg：", len(duplicates_gpkg))

    duplicates_corridor = merged_shp_corridor[
        merged_shp_corridor.duplicated(subset="geometry", keep=False)
    ]
    ...
    # Spatial join
    merged_with_iso = gpd.sjoin(
        merged_all, world[["geometry", "ISO_A3"]], how="left", predicate="within"
    )
    ...
    non_intersected_from_merged = merged_all.loc[
        ~merged_all.index.isin(df_africa_ports_proj.index)
    ]
    # Delete the airport and inland port
    non_intersected_from_merged = non_intersected_from_merged[
        non_intersected_from_merged["FeatureUID"] != "LTT02"
    ]
    non_intersected_from_merged = non_intersected_from_merged[
        non_intersected_from_merged["FeatureUID"] != "KMI01"
    ]
