...
def find_ref_mineral(x, m):
    if (
        m in str(x["commodity_group_export"]).lower()
        or m in str(x["commodity_subgroup_export"]).lower()
    ):
        return 1
    else:
        return 0
...
def main(config):
    ...
    port_corridor.rename(
        columns={"Commodities_traded_or_transported": "commodity_subgroup"},
        inplace=True,
    )
    port_corridor["corridor_annual_capacity_tons"] = port_corridor.progress_apply(
        lambda x: ";".join(["0"] * len(x["commodity_subgroup"].split(";"))), axis=1
    )
    ...
    usgs_ports["usgs_annual_capacity_tons"] = (
        usgs_ports["DsgAttr05"] * usgs_ports["capacity_convert_tons"]
    )
    usgs_ports.rename(
        columns={
            "DsgAttr03": "commodity_group_export",
    ...
    for col in columns_merge:
        df.append(
            usgs_ports.groupby(["FeatureUID"])[col].agg(
    ...
    port_matches["commodity_group_export"] = (
        port_matches[["commodity_group_export", "commodity_subgroup"]]
        .apply(lambda x: ";".join(x), axis=1)
        .str.replace("none", "")
        .str.strip(";")
    )
    port_matches["commodity_subgroup_export"] = (
        port_matches[["commodity_subgroup_export", "commodity_subgroup"]]
        .apply(lambda x: ";".join(x), axis=1)
        .str.replace("none", "")
        .str.strip(";")
    )
    port_matches["annual_capacity_tons"] = (
        port_matches[["usgs_annual_capacity_tons", "corridor_annual_capacity_tons"]]
        .apply(lambda x: ";".join(x), axis=1)
        .str.replace("none", "")
        .str.strip(";")
    )
    ...
    for col in [
        ...
        df.append(
            port_matches.groupby(["id"])[col].agg(
                [(col, lambda x: ";".join(map(str, x)))]
            )
        )
    ...
    for vt in vessel_types:
        ...
        port_matches.rename(
            columns={
                "annual_vessel_capacity_tons": f"{vt.lower().replace(' ', '_')}_annual_vessel_capacity_tons"
            },
            inplace=True,
        )
        ...
        all_ports_utilisations.append(
            pd.merge(
                p_df, df[["id", "annual_vessel_capacity_tons"]], how="left", on=["id"]
            ).fillna(0)
        )

    all_ports_utilisations = pd.concat(
        all_ports_utilisations, axis=0, ignore_index=True
    )
