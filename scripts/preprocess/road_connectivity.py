...
def main(config):
    ...
    road_edges = gpd.read_parquet(
        os.path.join(incoming_data_path, "africa_roads", "edges_with_topology.gpq")
    )
    road_nodes = gpd.read_parquet(
        os.path.join(incoming_data_path, "africa_roads", "nodes_with_topology.gpq")
    )
    ...
    main_roads = road_edges[road_edges[road_type_column].isin(main_road_types)][
        road_id_column
    ].values.tolist()
    ...
    for m_c in countries:
        country_roads = road_edges[
            (road_edges["from_iso_a3"] == m_c) | (road_edges["to_iso_a3"] == m_c)
        ]
    ...
    nearest_nodes.to_parquet(
        os.path.join(
            processed_data_path, "infrastructure", "africa_roads_nodes.geoparquet"
        )
    )
    edges.to_parquet(
        os.path.join(
