...
@click.command()
@click.option("--population", required=True, type=click.Path(exists=True))
...
@click.option("--output-main-roads", required=True, type=click.Path())
def main(
    population,
    airports,
    maritime,
    ...
    """Connect points of interest to the OpenStreetMap road network"""
    ...
    road_nodes_df.rename(columns={"id": node_id_column}, inplace=True)
    road_edges_df = road_edges_df.to_crs(epsg=epsg_meters)
    road_edges_df["length_m"] = road_edges_df.geometry.length
    road_nodes_df = road_nodes_df.to_crs(epsg=epsg_meters)

    main_roads = road_edges_df[road_edges_df[road_type_column].isin(main_road_types)][
        road_id_column
    ].values.tolist()
    ...
    for location in location_attributes:
        ...
        if location["type"] in (
            ...
            location_df = location_df[
                location_df[location["node_type_column"]].isin(location["node_type"])
            ]
        ...
        location_df.rename(
            columns={id_col: "location_id", iso_col: "location_iso"}, inplace=True
        )
    ...
    for m_c in countries:
        country_roads = road_edges_df[
            (road_edges_df["from_iso_a3"] == m_c) | (road_edges_df["to_iso_a3"] == m_c)
        ]
        connected_nodes = list(
            set(
                country_roads.from_id.values.tolist()
                + country_roads.to_id.values.tolist()
            )
        )
        country_nodes = road_nodes_df[
            road_nodes_df[node_id_column].isin(connected_nodes)
        ]
        if len(country_roads.index) > 0:
                ...
                if len(location_df.index) > 0:
                    ...
                    if len(location_df.index) > 0:
                        if key == "Polygon":
                            ...
                            loc_intersects = gpd.sjoin_nearest(
                                location_df,
                                country_roads[
                                    [road_id_column, road_type_column, "geometry"]
                                ],
                                how="left",
                            ).reset_index()
                            # get the intersected roads which are not the main roads
                            selected_edges = list(
                                set(loc_intersects[road_id_column].values.tolist())
                            )
                            polygon_roads = country_roads[
                                country_roads[road_id_column].isin(selected_edges)
                            ]
                            targets += list(
                                set(
                                    polygon_roads.from_id.values.tolist()
                                    + polygon_roads.to_id.values.tolist()
                                )
                            )

                            del selected_edges, polygon_roads
                        else:
                            loc_intersects = ckdnearest(
                                location_df, country_nodes[[node_id_column, "geometry"]]
                            )
                            targets += list(
                                set(loc_intersects[node_id_column].tolist())
                            )
                        del loc_intersects
            ...
            if len(targets) > 0:
                ...
                A = sorted(
                    graph.connected_components().subgraphs(),
                    key=lambda l: len(l.es[road_id_column]),
                    reverse=True,
                )
                for j in range(len(A)):
                    connected_edges = A[j].es[road_id_column]
                    connected_roads_df = country_roads[
                        country_roads[road_id_column].isin(connected_edges)
                    ]
                    connected_nodes = list(
                        set(
                            connected_roads_df.from_id.values.tolist()
                            + connected_roads_df.to_id.values.tolist()
                        )
                    )
                    sinks = [t for t in targets if t in connected_nodes]
                    if len(sinks) > 0:
                        source_df = connected_roads_df[
                            connected_roads_df[road_type_column].isin(main_road_types)
                        ]
                        if len(source_df.index) > 0:
                            source = source_df.from_id.values[0]
                            n_r, _ = network_od_path_estimations(
    ...
    nearest_roads = road_edges_df[road_edges_df[road_id_column].isin(nearest_roads)]
    nearest_roads = nearest_roads.to_crs(epsg=4326)
    connected_nodes = list(
        set(nearest_roads.from_id.values.tolist() + nearest_roads.to_id.values.tolist())
    )
