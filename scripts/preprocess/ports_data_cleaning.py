...
@click.option("--maritime-edges", required=True, type=click.Path(exists=True))
@click.option("--suez-network", required=True, type=click.Path(exists=True))
...
def main(
    ...
    new_ports_corridor = add_iso_code(new_ports_corridor, "Project_code", africa_adm0)
    new_ports_usgs = add_iso_code(new_ports_usgs, "FeatureUID", africa_adm0)
    new_ports_corridor["name"] = new_ports_corridor.progress_apply(
        lambda x: f"{x['Project_name']}_{x['Country']}", axis=1
    )
    new_ports_usgs["name"] = new_ports_usgs.progress_apply(
        lambda x: f"{x['FeatureNam']}_{x['Country']}", axis=1
    )

    new_ports = pd.concat(
        [new_ports_corridor, new_ports_usgs], axis=0, ignore_index=True
    )
    ...
    port_edges = gpd.read_file(maritime_edges)
    ...
    nearest_nodes["geometry"] = nearest_nodes.progress_apply(
        lambda x: add_lines(x, new_ports, pn, from_node_id, to_node_id), axis=1
    )
    ...
    port_edges = port_edges[
        ~(
            port_edges["from_id"].isin(remove_nodes)
            | port_edges["to_id"].isin(remove_nodes)
        )
    ]
    # Add the suez canal
    suez_canal_nodes = gpd.read_file(suez_network, layer="nodes")
    suez_canal_edges = gpd.read_file(suez_network, layer="edges")
    nodes = suez_canal_nodes.copy()
    nodes.rename(columns={"node_id": "to_id", "infra": "to_infra"}, inplace=True)

    edges = [port_edges, suez_canal_edges[["from_node", "to_node", "geometry"]]]
    ...
    # Create lines between nearest nodes
    suez_lines = pd.DataFrame(
        connect_pairs, columns=["from_id", "to_id", "from_infra", "to_infra"]
    )
    ...
    port_edges = gpd.GeoDataFrame(
        pd.concat(edges, axis=0, ignore_index=True),
        geometry="geometry",
        crs="EPSG:4326",
    )
    ...
    port_nodes = gpd.GeoDataFrame(
        pd.concat(
            [port_nodes, suez_canal_nodes[["id", "infra", "geometry"]]],
            axis=0,
            ignore_index=True,
        ),
        geometry="geometry",
    ...
    all_edges = []
    africa_nodes = port_nodes[port_nodes["Continent_Code"] == "AF"][
        "id"
    ].values.tolist()
    ...
    port_edges["from_infra"] = port_edges.progress_apply(
        lambda x: re.sub("[^a-zA-Z]+", "", x["from_id"]), axis=1
    )
    port_edges["to_infra"] = port_edges.progress_apply(
        lambda x: re.sub("[^a-zA-Z]+", "", x["to_id"]), axis=1
    )
    port_edges["id"] = port_edges.index.values.tolist()
    port_edges["id"] = port_edges.progress_apply(
        lambda x: f"maritimeroute_{x.id}", axis=1
    )
    port_edges["duplicates"] = (
        pd.DataFrame(np.sort(port_edges[["from_id", "to_id"]]))
