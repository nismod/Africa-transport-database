...
def main():
    ...
    mask_port = (ports_nodes["vessel_count_total"] == 0) & (
    ...
    ports_nodes.loc[~mask_port & ~mask_maritime, "source"] = ports_nodes.loc[
        ~mask_port & ~mask_maritime
    ].apply(lambda _: source_other, axis=1)
    ports_edges["source"] = [
        [
            "Verschuur, J. Global multi-hazard risk to port infrastructure and trade. Mendeley Data. V1, doi: 10.17632/kdyt24tsh5.1 (2022).",
            "Modelled",
    ...
    multi_df["source"] = [["Modelled"]] * len(multi_df)
    ...
    roads_edges_df.loc[mask_corridors, "source"] = roads_edges_df.loc[
        mask_corridors
    ].apply(lambda _: source_corridors_edges, axis=1)

    roads_edges_df.loc[mask_other, "source"] = roads_edges_df.loc[mask_other].apply(
        lambda _: source_other_edges, axis=1
    )
    ...
    rail_nodes_df.loc[mask_infranulls, "source"] = rail_nodes_df.loc[
        mask_infranulls
    ].apply(lambda _: source_infranull, axis=1)

    rail_nodes_df.loc[mask_other_nodes, "source"] = rail_nodes_df.loc[
        mask_other_nodes
    ...
    rail_edges_df.loc[mask_rail_edges1, "source"] = rail_edges_df.loc[
        mask_rail_edges1
    ].apply(lambda _: source_rail_edges1, axis=1)

    rail_edges_df.loc[mask_rail_edges2, "source"] = rail_edges_df.loc[
        mask_rail_edges2
    ...
    iww_df_nodes.loc[mask_nile, "source"] = iww_df_nodes.loc[mask_nile].apply(
        lambda _: [
    ...
    iww_df_nodes.loc[mask_iww, "source"] = iww_df_nodes.loc[mask_iww].apply(
        lambda _: ["Modelled"], axis=1
    )

    iww_df_nodes.loc[mask_osm, "source"] = iww_df_nodes.loc[mask_osm].apply(
        lambda _: [
            "Africa Geoportal. OpenStreetMap Waterways for Africa. https://africageoportal.maps.arcgis.com/home/item.html?id=82232d0415c04e7086414dff7eb1310f"
        ],
        axis=1,
    )
    ...
    mask_victoria = iww_df_edges["from_waterbody"].isin(victoria_lakes) | iww_df_edges[
        "to_waterbody"
    ].isin(victoria_lakes)

    mask_modelled = iww_df_edges["from_waterbody"].isin(modelled_lakes) | iww_df_edges[
        "to_waterbody"
    ].isin(modelled_lakes)

    mask_osm = ~mask_victoria & ~mask_modelled

    iww_df_edges.loc[mask_victoria, "source"] = iww_df_edges.loc[mask_victoria].apply(
        lambda _: [
    ...
    iww_df_edges.loc[mask_modelled, "source"] = iww_df_edges.loc[mask_modelled].apply(
        lambda _: [
            "Modelled",
    ...
    iww_df_edges.loc[mask_osm, "source"] = iww_df_edges.loc[mask_osm].apply(
        lambda _: [
            "Africa Geoportal. OpenStreetMap Waterways for Africa. https://africageoportal.maps.arcgis.com/home/item.html?id=82232d0415c04e7086414dff7eb1310f"
        ],
        axis=1,
