...
def main(config):
    ...
    grouped_data = (
        gdf_exploded.groupby(
            ["corridor_name", "tag_highway", "asset_type", "lanes", "paved"]
        )["length_km"]
        .sum()
        .reset_index()
    )
    grouped_data.loc[
        ~grouped_data["tag_highway"].isin(
            ["trunk", "motorway", "primary", "secondary", "tertiary", "bridge"]
        ),
        "tag_highway",
    ] = "tertiary"
    grouped_data["cost_tag"] = np.where(
        grouped_data["asset_type"] == "road_bridge",
        "bridge",
    ...
    merged_data = grouped_data.merge(
        costs_df,
        left_on=["cost_tag", "paved"],
        right_on=["tag_highway", "paved"],
        how="left",
    )
    ...
    merged_data["min_capital_cost_USD_2025"] = (
        merged_data["length_km"]
        * merged_data["lanes"]
        * merged_data["cost_min"]
        * (1.37)
    )
    merged_data["max_capital_cost_USD_2025"] = (
        merged_data["length_km"]
        * merged_data["lanes"]
        * merged_data["cost_max"]
        * (1.37)
    )
    merged_data["median_capital_cost_USD_2025"] = (
        merged_data["length_km"]
        * merged_data["lanes"]
        * merged_data["cost_median"]
        * (1.37)
    )

    merged_data.drop(["cost_min", "cost_max"], axis=1, inplace=True)
    merged_data["min_OM_cost_USD_2025"] = (
        merged_data["length_km"]
        * merged_data["lanes"]
        * costs_df.loc[costs_df["cost_type"] == "O&M", "cost_min"].values[0]
        * (1.8)
    )
    merged_data["max_OM_cost_USD_2025"] = (
        merged_data["length_km"]
        * merged_data["lanes"]
        * costs_df.loc[costs_df["cost_type"] == "O&M", "cost_max"].values[0]
        * (1.8)
    )
    merged_data["median_OM_cost_USD_2025"] = (
        merged_data["length_km"]
        * merged_data["lanes"]
    ...
    merged_data["investment_min_USD_2025"] = merged_data[
        "min_capital_cost_USD_2025"
    ] + (merged_data["min_OM_cost_USD_2025"]) * sum(maintain_years)
    merged_data["investment_max_USD_2025"] = merged_data[
        "max_capital_cost_USD_2025"
    ] + (merged_data["max_OM_cost_USD_2025"]) * sum(maintain_years)
    merged_data["investment_median_USD_2025"] = merged_data[
        "median_capital_cost_USD_2025"
    ] + (merged_data["median_OM_cost_USD_2025"]) * sum(maintain_years)

    merged_data = (
        merged_data.groupby(["corridor_name"])
        .agg(
            {
                "length_km": "sum",
