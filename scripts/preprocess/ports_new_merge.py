...
def modify_distance(x):
    if x["length"] < 355 and x["distance"] < 40075:
        ...
        return haversine(
            (round(start[1], 2), round(start[0], 2)),
            (round(end[1], 2), round(end[0], 2)),
        )
...
def match_ports(df1, df2, df1_id_column, df2_id_column, cutoff_distance):
    ...
    return selection[[df1_id_column, df2_id_column]], df1[
        ~df1[df1_id_column].isin(matched_ids)
    ]
...
def main(
    ...
    merged_gdf = merged_gdf.merge(
        maritime_values_cap, on="portid", how="left", suffixes=("", "_csv")
    )
    merged_gdf = merged_gdf.merge(
        maritime_values_turn, on="portid", how="left", suffixes=("", "_csv")
    )
    ...
    unmatched_right = right_join[right_join.index_right.isna()].drop(
        columns="index_right"
    )
    ...
    nodes_merged["infra_left"] = nodes_merged["infra_left"].fillna(
        nodes_merged["infra_right"]
    )
    nodes_merged["infra_left"] = nodes_merged["infra_left"].fillna(
        nodes_merged["infra"]
    )
    nodes_merged["iso3_left"] = nodes_merged["iso3_left"].fillna(
        nodes_merged["iso3_right"]
    )
    nodes_merged["country_left"] = nodes_merged["country_left"].fillna(
        nodes_merged["country_right"]
    )
    nodes_merged["continent_left"] = nodes_merged["continent_left"].fillna(
        nodes_merged["continent_right"]
    )
    ...
    nodes_merged.drop(
        columns=["name", "iso3_right", "infra_right", "infra", "id"],
        inplace=True,
        errors="ignore",
    )
    nodes_merged.rename(
    ...
    left_join.rename(columns={"id": "to_id", "geometry": "to_geometry"}, inplace=True)

    left_join["geometry"] = left_join.progress_apply(
        lambda x: LineString([x.from_geometry, x.to_geometry]), axis=1
    )
    left_join.drop(["from_geometry", "to_geometry"], axis=1, inplace=True)

    left_join = gpd.GeoDataFrame(
        left_join, geometry="geometry", crs=f"EPSG:{epsg_meters}"
    )
    ...
    port_edges = gpd.GeoDataFrame(df_edges, geometry="geometry", crs="EPSG:4326")

    port_edges["id"] = port_edges.index.values.tolist()
    port_edges["id"] = port_edges.progress_apply(
        lambda x: f"maritimeroute_{x.id}", axis=1
    )
    ...
    africa_edges = africa_edges.drop_duplicates(
        subset=["from_id", "to_id"], keep="first"
    )
    africa_edges = gpd.GeoDataFrame(
        pd.merge(africa_edges, port_edges, how="left", on=["from_id", "to_id"]),
        geometry="geometry",
        crs="EPSG:4326",
    )
    ...
    all_nodes = list(
        set(
            africa_edges["from_id"].values.tolist()
            + africa_edges["to_id"].values.tolist()
        )
    )
    ...
    africa_nodes = africa_nodes.rename(
        columns={
            "vessel_cou": "vessel_count_total",
            "vessel_c_1": "vessel_count_container",
            "vessel_c_2": "vessel_count_drybulk",
            "vessel_c_3": "vessel_count_generalcargo",
            "vessel_c_4": "vessel_count_roro",
            "vessel_c_5": "vessel_count_tanker",
            "industry_t": "industry_top1",
            "industry_1": "industry_top2",
            "industry_2": "industry_top3",
            "share_coun": "share_country_maritime_import",
            "share_co_1": "share_country_maritime_export",
            "capacity_container": "capacity_container_tons",
            "capacity_drybulk": "capacity_drybulk_tons",
            "capacity_generalcargo": "capacity_generalcargo_tons",
            "capacity_roro": "capacity_roro_tons",
            "capacity_tanker": "capacity_tanker_tons",
            "time_container": "time_container_h",
            "time_drybulk": "time_drybulk_h",
            "time_generalcargo": "time_generalcargo_h",
            "time_roro": "time_roro_h",
            "time_tanker": "time_tanker_h",
        }
    )
