...
def remove_nodes_and_edges(
    ...
    port_commodities_df = pd.read_csv(
        os.path.join(data_path, "port_statistics", "port_known_commodities_traded.csv")
    )
...
def main(config):
    ...
    if plot_mine_sites is True:
        mine_sites_df = gpd.read_file(
            os.path.join(
                data_path, "Minerals", "copper_mines_tons_refined_unrefined.gpkg"
            )
        )
        mine_sites_df["geometry"] = mine_sites_df.geometry.centroid
        mine_sites_df["refining_type"] = np.where(
            mine_sites_df["process_binary"] == 0, "Unrefined", "Refined"
        )
    ...
    if plot_mine_sites is True:
        ...
        mines_df = gpd.read_file(
            os.path.join(data_path, "minerals", "ccg_mines_est_production.gpkg")
        )
        ...
        if mine_id_col not in mines_df.columns.values.tolist():
            ...
            mines_df[mine_id_col] = mines_df.progress_apply(
                lambda x: f"mine_{x[mine_id_col]}", axis=1
            )
        for reference_mineral in reference_minerals:
            if (
                f"{reference_mineral}_processed_ton"
                not in mines_df.columns.values.tolist()
            ):
                mines_df[f"{reference_mineral}_processed_ton"] = 0
            if (
                f"{reference_mineral}_unprocessed_ton"
                not in mines_df.columns.values.tolist()
            ):
                mines_df[f"{reference_mineral}_unprocessed_ton"] = 0

            mines_df[f"{reference_mineral}_tons"] = (
                mines_df[f"{reference_mineral}_processed_ton"]
                + mines_df[f"{reference_mineral}_unprocessed_ton"]
            )
        ...
        all_mines["total_tons"] = 1.0e-3 * all_mines[
            [f"{rf}_tons" for rf in reference_minerals]
        ].sum(axis=1)
    ...
    plot_mine_sites = False
    if plot_mine_sites is True:
        s_and_p_mines = gpd.read_file(
            os.path.join(data_path, "Minerals", "s_and_p_mines.gpkg")
        )
        mineral = "Copper"
        s_and_p_mines["refining_type"] = np.where(
            s_and_p_mines["PROCESSING_METHODS"].isna(), "Unrefined", "Refined"
        )
    ...
    if plot_flows is True:
        ...
        for year in years:
            flow_df = gpd.read_file(
                os.path.join(output_path, "flow_mapping", f"edges_flows_{year}.gpkg"),
                layer=mineral_class,
            )
            flow_df, _ = remove_nodes_and_edges(
            ...
            for idx, (oc, ot, ocl) in enumerate(
                ...
                if oc in flow_df.columns.values.tolist():
                    ...
                    if max(values_range) > 0:
                        ...
                        save_fig(
                            os.path.join(
                                figures, f"ccg_{mineral_class}_{ot}_flows_{year}.png"
                            )
                        )
                        plt.close()
    ...
    if plot_flows is True:
        ...
        for year in years:
            flow_df = gpd.read_file(
                os.path.join(
                    output_path, "flow_mapping", f"{mineral_class}_flows_{year}.gpkg"
                ),
                layer="edges",
            )
    ...
    if plot_flows is True:
        ...
        for year in years:
            ...
            nodes_df = gpd.read_file(
                os.path.join(output_path, "flow_mapping", f"nodes_flows_{year}.gpkg"),
                layer=mineral_class,
            )
            nodes_df = nodes_df[nodes_df["iso3"].isin(ccg_isos)]
            flow_df, nodes_df = remove_nodes_and_edges(
