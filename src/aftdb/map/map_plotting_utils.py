...
def line_map_plotting(
    ...
    width_by_range = generate_weight_bins(
        weights, width_step=width_step, n_steps=line_steps, interpolation="log"
    )
    # print (width_by_range)
    line_geoms_by_category = {"1": [], "2": []}
    for record in df.itertuples():
    ...
    styles = OrderedDict(
        [
            ...
            ("2", Style(color="#969696", zindex=7, label=f"No {value_label}")),
        ]
    )
...
def point_map_plotting(
    ...
    legend_handles = create_figure_legend(
        ...
        max_weight,
        "marker",
        [point_color] * len(width_by_range),
        10,
        marker=marker,
    )
    if plot_title:
    ...
    first_legend = ax.legend(
        handles=legend_handles, fontsize=9, title=legend_label, loc="lower left"
    )
    ax.add_artist(first_legend)
    print("* Plotting ", plot_title)
    legend_from_style_spec(ax, styles, fontsize=8, loc="lower right")
...
def line_map_plotting_colors_width(
    ...
    style_noflood = [
        (str(0), Style(color=no_value_color, zindex=7, label=f"No {value_label}"))
    ]
    styles = OrderedDict(
        style_noflood
        + [
            (str(j + 1), Style(color=line_colors[j], zindex=8 + j, label=None))
...
def point_map_plotting_color_width(
    ax,
    df,
    df_column,
    weights,
    ...
    styles = OrderedDict(
        style_noflood
        + [
            (str(j + 1), Style(color=point_colors[j], zindex=8 + j, label=None))
            for j in range(len(point_colors))
        ]
    )
    ...
    if plot_title:
        plt.title(plot_title, fontsize=9)
    first_legend = ax.legend(
        handles=legend_handles, fontsize=11, title=f"{legend_label}", loc="lower left"
    )
    ax.add_artist(first_legend)
    print("* Plotting ", plot_title)
