...
def create_edges_from_nearest_node_joins(
    from_df,
    to_df,
    from_id_column,
    to_id_column,
    ...
    from_df.rename(
        columns={from_id_column: "from_id", from_iso_column: "from_iso_a3"},
        inplace=True,
    )
    to_df.rename(
        columns={to_id_column: "to_id", to_iso_column: "to_iso_a3"}, inplace=True
    )
    from_df["from_infra"] = from_mode
    to_df["to_infra"] = to_mode

    from_to_df = ckdnearest(
        from_df[["from_id", "from_iso_a3", "from_infra", "geometry"]],
    ...
    if len(from_to_df.index) > 0:
        ...
        from_to_df = pd.merge(
            from_to_df, to_df[["to_id", "to_geometry"]], how="left", on=["to_id"]
        )
